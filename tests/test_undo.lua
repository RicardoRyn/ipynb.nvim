...
--------------------------------------------------------------------------------
-- Test 1.6: Multiple Insert Sessions Within Single Edit Float
-- Enter cell, insert mode -> edit -> normal, insert mode -> edit -> normal
-- Undo twice while still in edit float to verify granular undo blocks.
-- Expected: Each insert session creates a separate undo block.
--------------------------------------------------------------------------------
h.run_test("multiple_insert_sessions_in_edit_float", function()
	h.open_notebook("simple.ipynb")
	local original = h.get_cell_content(1)

	-- Enter edit float (normal mode in float)
	h.enter_cell(1)
	h.assert_true(h.is_in_edit_float(), "Should be in edit float")

	-- First insert session
	local first_edit
	vim.api.nvim_feedkeys("gg", "mtx", false)
	h.insert_text_and_esc("FIRST EDIT", "O")
	first_edit = h.get_edit_buffer_content()

	-- Verify first edit took effect (edit buffer)
	h.assert_true(first_edit:match("FIRST EDIT"), "Edit buffer should contain FIRST EDIT")

	-- Second insert session
	local second_edit
	vim.api.nvim_feedkeys("gg", "mtx", false)
	h.insert_text_and_esc("SECOND EDIT", "O")
	second_edit = h.get_edit_buffer_content()

	-- Verify second edit took effect (edit buffer)
	h.assert_true(second_edit:match("SECOND EDIT"), "Edit buffer should contain SECOND EDIT")
	h.assert_true(second_edit:match("FIRST EDIT"), "Edit buffer should still contain FIRST EDIT")

	-- Still in edit float
	h.assert_true(h.is_in_edit_float(), "Should still be in edit float")

	-- First undo: should revert second insert session only
	h.undo()
	local after_undo1 = h.get_edit_buffer_content()
	h.assert_false(after_undo1:match("SECOND EDIT"), "SECOND EDIT should be undone")
	h.assert_true(after_undo1:match("FIRST EDIT"), "FIRST EDIT should remain")

	-- Verify edit buffer also reflects the undo
	local edit_after_undo1 = h.get_edit_buffer_content()
	h.assert_false(edit_after_undo1:match("SECOND EDIT"), "Edit buffer: SECOND EDIT should be undone")
	h.assert_true(edit_after_undo1:match("FIRST EDIT"), "Edit buffer: FIRST EDIT should remain")

	-- Second undo: should revert first insert session
	h.undo()
	local after_undo2 = h.get_edit_buffer_content()
	h.assert_false(after_undo2:match("FIRST EDIT"), "FIRST EDIT should be undone")
	h.assert_eq(after_undo2, original, "Should be back to original content")

	-- Verify edit buffer matches original
	local edit_after_undo2 = h.get_edit_buffer_content()
	h.assert_eq(edit_after_undo2, original, "Edit buffer should be back to original")
end)

--------------------------------------------------------------------------------
-- Test 1.7: Undo Across Different Cells
